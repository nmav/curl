...
static CURLcode myssh_getworkingpath(struct connectdata *conn, char *homedir,   /* when SFTP is used */
                                     char **path);
...
static int myssh_getsock(struct connectdata *conn, curl_socket_t * sock,        /* points to numsocks number
                                                                                   of sockets */
                         int numsocks);
                                      ...
                                      bool disconnect)
{
  ...
  while ((sshc->state != SSH_STOP) && !result) {
    ...
    if (!result && block) {
      ...
      (void) Curl_socket_check(fd_read, CURL_SOCKET_BAD, CURL_SOCKET_BAD, left > 1000 ? 1000 : left);   /* ignore result */
    }
